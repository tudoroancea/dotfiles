...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		renderResult(result, { expanded, isPartial }, theme, context) {
			...
			return new Text(theme.fg(context.isError ? "error" : "dim", context.isError ? `failed · ${summary}` : summary), 0, 0);
		},
	});
	...
	pi.registerTool({
		...
		renderResult(result, { expanded, isPartial }, theme, context) {
			...
			if (context.isError || !diff) {
				...
				return new Text(theme.fg(context.isError ? "error" : "success", message || "applied"), 0, 0);
			}
